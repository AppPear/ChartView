...
public struct ChartLabel<Root: ChartDataPoint, ChartValueType: ChartValue>: View where Root == ChartValueType.Root {
    @EnvironmentObject var chartValue: ChartValueType
    @EnvironmentObject var chartData: ChartData<Root>
        ...
        HStack {
            Text(textToDisplay)
                ...
                .onReceive(self.chartValue.objectWillChange) { _ in
                    self.textToDisplay = self.chartValue.interactionInProgress ? String(format: format, Double(self.chartValue.currentValue?[keyPath: chartData.keyPathForGraphValue] ?? 0.0)) : self.title
                }
